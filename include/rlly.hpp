 ...
 */
template <typename S> class StepResult {
  public:
...
template <typename S> StepResult<S>::StepResult()
{
    next_state = -1;
    reward     = 0;
    done       = false;
...
template <typename S> StepResult<S>::StepResult(S _next_state, double _reward, bool _done)
{
...
class Random {
  ...
  public:
     ...
     * @details The parameter prob is passed by reference to avoid copying. It is not changed by the
     * algorithm.
 ...
 * @param bins 2d array such that bins[i] represents the intervals where to search for d_val[i].
 * Represents a set of hypercubes in R^d
...
class Params {
  ...
  public:
    ...
    void append(std::string param_name, int param_val) { int_params[param_name] = param_val; };
...
template <typename S> class RenderInterface2D {
  private:
    /* data */
  public:
    RenderInterface2D() {};
    ...
    */
     ...
     */
    virtual utils::render::Scene2D get_background_for_render2d()
    {
        return utils::render::Scene2D();
    };
 ...
 */
template <typename S_space, typename A_space> class Env {
  public:
    Env() {};
    virtual ~Env() {};
     ...
     */
    virtual void set_state(S_type state)
    {
        throw "Error: set_state() not implemented for this environment.";
    };
...
template <typename S_space, typename A_space> void Env<S_space, A_space>::set_seed(uint _seed)
{
    if (_seed == 0) {
        _seed = std::rand();
    }
...
class DiscreteReward {
  ...
  public:
     ...
     */
    DiscreteReward(utils::vec::vec_3d _mean_rewards,
                   std::string _noise_type,
                   std::vector<double> _noise_params);
    ~DiscreteReward() {};
...
class FiniteMDP : public FiniteEnv {
  ...
  public:
     ...
     */
    FiniteMDP(DiscreteReward _reward_function,
              utils::vec::vec_3d _transitions,
              int _default_state = 0,
              int _seed          = -1);
    ...
    FiniteMDP(DiscreteReward _reward_function,
              utils::vec::vec_3d _transitions,
              std::vector<int> _terminal_states,
              int _default_state = 0,
              int _seed          = -1);
  ...
  protected:
     ...
     */
    void set_params(DiscreteReward _reward_function,
                    utils::vec::vec_3d _transitions,
                    int _default_state = 0,
                    int _seed          = -1);
...
/**
 * CartPole environment, as in
 * https://github.com/openai/gym/blob/master/gym/envs/classic_control/cartpole.py
 */
class CartPole : public ContinuousStateEnv,
                 public rlly::utils::render::RenderInterface2D<std::vector<double>> {
  private:
 ...
 * @details
 *      The agent starts at (start_x, start_y) and, in each state, it can take for actions (0 to 3)
 * representing a displacement of (-d, 0), (d, 0), (0, -d) and (0, d), respectively.
 *
 ...
 *
 */
class ChangingSquareWorld : public ContinuousStateEnv,
                            public rlly::utils::render::RenderInterface2D<std::vector<double>> {
  private:
  ...
  public:
    ChangingSquareWorld(int _period,
...
ContinuousStateEnv* make_continuous_state_env(std::string env_name,
                                              utils::params::Params* env_params = nullptr);
...
class MountainCar : public ContinuousStateEnv,
  ...
  public:
     ...
     */
    enum StateLabel { position = 0, velocity = 1 };
 ...
 *   Fail probability:
 *
 *      With probability fail_p, a random action will be taken instead of the chosen action. Note
 * that, even in the case of failure, the chosen action can be chosen by chance.
 */
class GridWorld : public FiniteMDP, public rlly::utils::render::RenderInterface2D<int> {
  ...
  public:
     ...
     * @param reward_smoothness reward parameter. default = 0, mean_reward[s, a, s'] =
     * exp(-(distance(s', goal_state)/reward_smoothness)^2  )
     * @param reward_sigma standard deviation of the reward noise. reward(s, a, s') = mean_reward(s,
     * a, s') + reward_sigma*standard_gaussian_noise
     */
    GridWorld(int _nrows,
              int _ncols,
              double fail_p            = 0,
              double reward_smoothness = 0,
              double reward_sigma      = 0);
    ~GridWorld() {};
};
} // namespace env
} // namespace rlly
 ...
 */
class WallSquareWorld : public ContinuousStateEnv,
                        public rlly::utils::render::RenderInterface2D<std::vector<double>> {
  private:
    // Coordinates of start position
    double start_x = 0.1;
 ...
 *
 */
class ChangingWallSquareWorld : public ContinuousStateEnv,
                                public rlly::utils::render::RenderInterface2D<std::vector<double>> {
  private:
    // Coordinates of start position
    double start_x = 0.1;
 ...
 *
 *      The immediate reward received in each state s = (s_x, s_y) is, for any action a,
 *          r(s, a) =  \sum_i c_i *  exp( - ((s_x-blob_i_x)^2 +
 * (s_y-blob_i_y)^2)/(2*reward_smoothness^2)  )
 *
 *      Every <period> episodes there is a change in the blob configuration.
 *
 */
class ChasingBlobs : public ContinuousStateEnv,
                     public rlly::utils::render::RenderInterface2D<std::vector<double>> {
  private:
    // Coordinates of start position
    double start_x = 0.5;
    ...
    std::vector<std::vector<double>> reward_multipliers = {{0.25, 0.00, 0.00, 0.00},
                                                           {0.25, 0.50, 0.00, 0.00},
                                                           {0.25, 0.50, 0.75, 0.00},
                                                           {0.25, 0.50, 0.75, 1.00}};
 ...
 * @tparam EnvType must NOT be abstract, we need to instantiate it to infer the types of state and
 * action spaces
 */
template <typename EnvType> class IsomorphicWrapper : public EnvType {
  private:
    EnvType foo_env;
  ...
  public:
    ...
    // reset
    virtual S_type reset() override { return p_env.reset(); };

    // step
    virtual env::StepResult<S_type> step(A_type action) override { return p_env.step(action); };
...
template <typename EnvType> void IsomorphicWrapper<EnvType>::set_seed(int _seed)
{
...
class DiscretizeStateWrapper : public BaseWrapper<EnvType, spaces::Discrete, spaces::Discrete> {
  ...
  public:
    ...
    /**
     * @param env
     * @param vec_n_bins vec_n_bins[i] is the number of intervals in the discretization of the i-th
    ...
    /**
     * all_bins[i] = {x_1, ..., x_n } the points corresponding to the discretization of the i-th
...
template <typename EnvType> void DiscretizeStateWrapper<EnvType>::init()
{
    if (this->p_env.observation_space.name != spaces::box) {
        std::cerr << "Error: DiscretizeStateWrapper requires an environment with an observation "
                     "space of type spaces::Box ."
                  << std::endl;
        throw;
    }
    if (this->p_env.action_space.name != spaces::discrete) {
        std::cerr << "Error: DiscretizeStateWrapper requires an environment with an action space "
                     "of type spaces::Discrete."
                  << std::endl;
        throw;
    }
    ...
    : BaseWrapper<EnvType, spaces::Discrete, spaces::Discrete>(env)
{
    ...
    for (unsigned int dd = 0; dd < dim; dd++) {
        ...
        for (int bin = 0; bin < n_bins + 1; bin++) {
            ...
            if (bin == n_bins)
                discretization[bin] += tol; // "close" the last interval
        }
    ...
    : BaseWrapper<EnvType, spaces::Discrete, spaces::Discrete>(env)
{
    ...
    if (vec_n_bins.size() != dim) {
        std::cerr << "Incompatible dimensions in the constructor of DiscretizeStateWrapper."
                  << std::endl;
        throw;
    }
    for (unsigned int dd = 0; dd < dim; dd++) {
...
template <typename EnvType> env::StepResult<int> DiscretizeStateWrapper<EnvType>::step(int action)
{
...
template <typename EnvType> int DiscretizeStateWrapper<EnvType>::reset()
{
 ...
 */
template <typename EnvType> void render_env(EnvType& env)
{
    if (env.rendering_enabled && env.rendering_type == "2d") {
        ...
        for (int ii = 0; ii < n_data; ii++) {
            utils::render::Scene2D scene =
                env.get_scene_for_render2d(env.state_history_for_rendering[ii]);
        ...
        std::cerr << "Error: environement " << env.id
                  << " is not enabled for rendering. Try calling env.enable_rendering()."
                  << std::endl;
    }
}
 ...
 * @details Based on the OpenGL tutorial at
...
FiniteEnv* make_finite_env(std::string env_name, utils::params::Params* env_params /*= nullptr*/)
{
    if (env_name == "GridWorld") {
        //  GridWorld(int _nrows, int _ncols, double fail_p = 0, double reward_smoothness = 0,
        //  double reward_sigma = 0)
        if (env_params != nullptr && env_params->is_defined("nrows", "int") &&
            env_params->is_defined("ncols", "int") && env_params->is_defined("fail_p", "double") &&
            env_params->is_defined("reward_smoothness", "double") &&
            env_params->is_defined("reward_sigma", "double")) {
            return new GridWorld(env_params->int_params["nrows"],
    ...
    std::cerr << " Error! Invalid name or missing parameters in make_finite_env. Returning nullptr"
              << std::endl;
    return nullptr;
}
...
ContinuousStateEnv* make_continuous_state_env(std::string env_name,
                                              utils::params::Params* env_params /*= nullptr*/)
{
    if (env_name == "SquareWorld")
        return new SquareWorld();
    else if (env_name == "WallSquareWorld")
        return new WallSquareWorld();
    else if (env_name == "MountainCar")
    ...
    std::cerr << " Error! Invalid name or missing parameters in make_continuous_state_env. "
                 "Returning nullptr"
              << std::endl;
    return nullptr;
}
...
ContinuousEnv* make_continuous_env(std::string env_name,
                                   utils::params::Params* env_params /*= nullptr*/)
{
    if (env_name == "MovingBall")
        return new MovingBall();

    std::cerr
        << " Error! Invalid name or missing parameters in make_continuous_env. Returning nullptr"
        << std::endl;
    return nullptr;
}
...
ContinuousActionEnv* make_continuous_action_env(std::string env_name,
                                                utils::params::Params* env_params /*= nullptr*/)
{
    std::cerr << " Error! Invalid name or missing parameters in make_continuous_action_env. "
                 "Returning nullptr"
              << std::endl;
    return nullptr;
}
...
MovingBall::MovingBall()
{
    ...
    A[0][2] = delta_t;
    A[1][3] = delta_t;
    B[2][0] = delta_t / ball_mass;
    B[3][1] = delta_t / ball_mass;
...
StepResult<std::vector<double>> MovingBall::step(std::vector<double> action)
{
    ...
    if (rendering_enabled)
        append_state_for_rendering(state);
    //
    double x  = state[0];
    double y  = state[1];
    double vx = state[2];
...
env::StepResult<std::vector<double>> ChangingWallSquareWorld::step(int action)
{
    ...
    bool done = false;
    double reward =
        std::exp(-0.5 * (std::pow(state[0] - goal_x, 2) + std::pow(state[1] - goal_y, 2)) /
                 (std::pow(reward_smoothness, 2)));
...
utils::render::Scene2D
ChangingWallSquareWorld::get_scene_for_render2d(std::vector<double> state_var)
{
...
ChangingSquareWorld::ChangingSquareWorld(int _period,
                                         bool _changing_reward /*= true*/,
                                         bool _changing_transition /*= true */)
    : period(_period), changing_reward(_changing_reward), changing_transition(_changing_transition)
{
    ...
    if (current_episode % period == 0) {
        ...
        if (changing_transition) {
            ...
            if (new_config != current_transition_configuration)
                ...
                current_transition_configuration =
                    (current_transition_configuration + 1) % n_configurations;
        }
        if (changing_reward) {
            int new_config = randgen.sample_int_uniform(0, n_configurations - 1);
            if (new_config != current_reward_configuration)
                ...
                current_reward_configuration =
                    (current_reward_configuration + 1) % n_configurations;
        }
    }
    // set initial state
    std::vector<double> initial_state{start_x, start_y};
...
int Random::choice(std::vector<double>& prob, double u /* = -1 */)
{
    int n = prob.size();
    if (n == 0) {
        std::cerr << "Calling Random::choice with empty probability vector! Returning -1."
                  << std::endl;
        return -1;
    }
    ...
    double unif_sample;
    if (u == -1) {
        unif_sample = real_unif_dist(generator);
    } else {
        unif_sample = u;
    }
...
uint Random::get_seed() { return seed; }
...
bool Discrete::contains(int x) { return (x >= 0 && x < n); }
...
env::StepResult<std::vector<double>> ChasingBlobs::step(int action)
{
    ...
    for (int ii = 0; ii < 4; ii++) {
        ...
        reward += c_ii *
                  std::exp(-0.5 *
                           (std::pow(state[0] - blob_ii_x, 2) + std::pow(state[1] - blob_ii_y, 2)) /
                           (std::pow(reward_smoothness, 2)));
    }
    ...
    if (action == 0)
        state[0] = std::max(0.0, state[0] - displacement);
    else if (action == 1)
        state[0] = std::min(1.0, state[0] + displacement);
    else if (action == 2)
        state[1] = std::max(0.0, state[1] - displacement);
    else if (action == 3)
        state[1] = std::min(1.0, state[1] + displacement);
...
utils::render::Scene2D ChasingBlobs::get_background_for_render2d()
{
    ...
    //         shape.type = "GL_QUADS";
    //         float reward = std::exp( -0.5*(std::pow(x-goal_x, 2) + std::pow(y-goal_y,
    //         2))/(std::pow(reward_smoothness, 2)));
...
double mean(std::vector<double> vec)
{
    ...
    int n         = vec.size();
    if (n == 0) {
        std::cerr << "Warning: calling mean() on empty vector." << std::endl;
    }
    for (int i = 0; i < n; i++) {
        result += vec[i];
    }
...
double stdev(std::vector<double> vec)
{
    int n = vec.size();
    if (n == 0) {
        std::cerr << "Warning: calling stdev() on empty vector." << std::endl;
    }
    double mu = mean(vec);
...
double inner_prod(std::vector<double> vec1, std::vector<double> vec2)
{
    ...
    if (n == 0) {
        std::cerr << "Warning: calling inner_prod() on empty vectors." << std::endl;
    }
    double result = 0.0;
...
StepResult<std::vector<double>> CartPole::step(int action)
{
    ...
    if (!done)
        ...
        if (steps_beyond_done == 0)
            std::cerr
                << "Warning (CartPole): undefined behaviour: calling step() after done = True."
                << std::endl;
        steps_beyond_done += 1;
        reward = 0.0;
    }
...
utils::render::Scene2D CartPole::get_scene_for_render2d(std::vector<double> state_var)
{
    ...
    float norm = std::sqrt(u_vec[0] * u_vec[0] + u_vec[1] * u_vec[1]);
    u_vec[0] /= norm;
    u_vec[1] /= norm;
...
std::unique_ptr<FiniteEnv> Chain::clone() const { return std::make_unique<Chain>(*this); }
...
utils::render::Scene2D SquareWorld::get_background_for_render2d()
{
    ...
    while (x < 1.0) {
        ...
        while (y < 1.0) {
            utils::render::Geometric2D shape;
            shape.type   = "GL_QUADS";
            float reward = std::exp(-0.5 * (std::pow(x - goal_x, 2) + std::pow(y - goal_y, 2)) /
                                    (std::pow(reward_smoothness, 2)));
...
FiniteMDP::FiniteMDP(DiscreteReward _reward_function,
                     utils::vec::vec_3d _transitions,
                     int _default_state /* = 0 */,
                     int _seed /* = -1 */)
{
...
void FiniteMDP::set_params(DiscreteReward _reward_function,
                           utils::vec::vec_3d _transitions,
                           int _default_state /* = 0 */,
                           int _seed /* = -1 */)
{
...
{
    return (std::find(terminal_states.begin(), terminal_states.end(), _state) !=
            terminal_states.end());
}
...
DiscreteReward::DiscreteReward() { noise_type = "none"; }
...
void Render2D::set_window_name(std::string name) { window_name = name; }

void Render2D::set_refresh_interval(int interval) { refresh_interval = interval; }
...
void Render2D::set_data(std::vector<utils::render::Scene2D> _data) { data = _data; }

void Render2D::set_background(utils::render::Scene2D _background) { background = _background; }
...
void Render2D::draw_geometric2d(utils::render::Geometric2D geom)
{
    ...
    if (geom.type == "GL_POINTS")
        glBegin(GL_POINTS);
    else if (geom.type == "GL_LINES")
        glBegin(GL_LINES);
    else if (geom.type == "GL_LINE_STRIP")
        glBegin(GL_LINE_STRIP);
    else if (geom.type == "GL_LINE_LOOP")
        glBegin(GL_LINE_LOOP);
    else if (geom.type == "GL_POLYGON")
        glBegin(GL_POLYGON);
    else if (geom.type == "GL_TRIANGLES")
        glBegin(GL_TRIANGLES);
    else if (geom.type == "GL_TRIANGLE_STRIP")
        glBegin(GL_TRIANGLE_STRIP);
    else if (geom.type == "GL_TRIANGLE_FAN")
        glBegin(GL_TRIANGLE_FAN);
    else if (geom.type == "GL_QUADS")
        glBegin(GL_QUADS);
    else if (geom.type == "GL_QUAD_STRIP")
        glBegin(GL_QUAD_STRIP);
    else
        std::cerr << "Error in Render2D::draw_geometric2d: invatid primitive type!" << std::endl;
...
GridWorld::GridWorld(int _nrows,
                     int _ncols,
                     double fail_p /* = 0 */,
                     double reward_smoothness /* = 0 */,
                     double reward_sigma /* = 0 */)
{
    ...
    for (int jj = 0; jj < S; jj++) {
        ...
        double squared_distance =
            std::pow((1.0 * next_state_coord[0] - 1.0 * goal_coord[0]) / (nrows - 1), 2) +
...
void GridWorld::render()
{
    ...
    for (auto const& cell : index2coord) // key = cell.first, value = cell.second
    {
        ...
        if (std::find(terminal_states.begin(), terminal_states.end(), cell.first) !=
            terminal_states.end())
            cell_str = " x  ";
...
void GridWorld::set_state(int state) { this->state = state; }
...
std::unique_ptr<FiniteEnv> GridWorld::clone() const { return std::make_unique<GridWorld>(*this); }
...
utils::render::Scene2D GridWorld::get_background_for_render2d()
{
    ...
    for (int cc = 0; cc < ncols; cc++) {
        for (int rr = 0; rr < nrows; rr++) {
            ...
            shape.type = "GL_QUADS";
            if ((rr + color) % 2 == 0)
                shape.set_color(0.35, 0.35, 0.35);
            else
                shape.set_color(0.5, 0.5, 0.5);
            if (rr == nrows - 1 && cc == ncols - 1)
                shape.set_color(0.0, 0.5, 0.0);
            float x = 1.0 * cc;
            float y = 1.0 * rr;
